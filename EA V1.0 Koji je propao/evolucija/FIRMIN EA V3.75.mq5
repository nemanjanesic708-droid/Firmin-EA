...
bool ORCH_HttpPostJson(string url, string json, string &resp)
{
   ...
   int status = WebRequest("POST", url, hdrs, 5000, data, http_result, headers);
   if(status==200)
   {
      resp = CharArrayToString(http_result, 0, -1, CP_UTF8);
      return true;
   }
   PrintFormat("ORCH POST failed: %d %s", status, headers);
   return false;
}
// === BEGIN ADAPTIVE_TP_BE_GLOBALS ===
...
bool ORCH_HttpGet(string url, string &resp)
{
   ...
   int status = WebRequest("GET", url, hdrs, 5000, data, http_result, headers);
   if(status==200){
      resp = CharArrayToString(http_result, 0, -1, CP_UTF8);
      return true;
   }
