...
// === Stability & ORCH globals (moved before use) ===
input bool OpenOnNewBarOnly      = true;
input int  DailyMaxTrades_input  = 8;
datetime g_LastOrderBarTime = 0;
int      g_TodayTradesCount = 0;
int      g_TodayDate        = -1;
int g_LastOpenBar = -1;
bool ORCH_ORCH_Enabled_runtime = true;
...
bool GateOnePerBarGlobal()
{
   int curBar  = (int)iBarShift(_Symbol,(ENUM_TIMEFRAMES)Period(),TimeCurrent(),true);
   int lastBar = (g_LastOrderBarTime>0 ? (int)iBarShift(_Symbol,(ENUM_TIMEFRAMES)Period(),g_LastOrderBarTime,true) : -9999);
   if(lastBar == curBar) return false;
   return true;
}
bool GateDailyCap()
{
   MqlDateTime __tm;
   TimeToStruct(TimeCurrent(), __tm);
   int today = (int)__tm.day;
   if(g_TodayDate != today){ g_TodayDate = today; g_TodayTradesCount = 0; }
   return (DailyMaxTrades_input<=0) ? true : (g_TodayTradesCount < DailyMaxTrades_input);
}
void MarkTradeOpened()
{
   g_LastOrderBarTime = iTime(_Symbol,(ENUM_TIMEFRAMES)Period(),0);
   MqlDateTime __tm2;
   TimeToStruct(TimeCurrent(), __tm2);
   int today = (int)__tm2.day;
   if(g_TodayDate != today){ g_TodayDate = today; g_TodayTradesCount = 0; }
   g_TodayTradesCount++;
   int curBar = (int)iBarShift(_Symbol,(ENUM_TIMEFRAMES)Period(),TimeCurrent(),true);
   g_LastOpenBar = curBar;
}
static bool g_opening_now = false;
bool BeginOpenAttempt(){
   if(g_opening_now) return false;
   g_opening_now = true;
   return true;
}
void EndOpenAttempt(){ g_opening_now = false; }





// === Stability patch (one-trade-per-bar & daily cap) ===



// === Tester / Optimization helpers ===
bool IsTestingOrOpt()
  {
...
// --- utils: Trim (MQL5 has StringTrimLeft/Right only)
...
// --- Logging inputs (added)
input bool   ExpertsLogging = true;
input int    LogLevel       = 3;    // 0=none, 1=errors, 2=warn, 3=info, 4=debug, 5=trace
input int    LogMinSecs     = 5;    // rate-limit seconds for info/debug




// === ORCH pretrade & logging prototypes ===
string ORCH_TimeframeToStr(int tf);
string ORCH_SessionName();
bool   ORCH_PretradeCheck(const string signal, double atr_pips, double spread_pips, const string session, bool news30m,
                          const string bars, string &request_id, double &rr, double &slmin, double &slmax, string &regime, string &notes);
bool   ORCH_TradeLogOpen(const string request_id, const string symbol, const string tf, const string side,
                         double lots, double sl_pips, double tp_pips, double rr_target, const string regime,
                         double spread_pips, double atr_pips, const string session, const string notes,
                         ulong ticket_id, double price_open);
bool   ORCH_TradeLogClose(const string request_id, const string symbol, const string side, ulong ticket_id,
                          double price_close, double pnl_currency, double pnl_pips, double duration_s);
// globals for last pretrade decision to attach to logs
string ORCH_last_req_id = "";
string ORCH_last_regime = "";
double ORCH_last_rr = 0.0;
double ORCH_last_sl_pips = 0.0;
double ORCH_last_tp_pips = 0.0;


// --- Helper: match magic across MSW modes
bool MagicMatches(long mg)
{
   // Match base magic or MSW range when switching is used
   if(mg==g_MagicNumber) return true;
   if(g_MSW_UseSwitch){
      if(mg>=g_MagicNumber && mg<=g_MagicNumber + MSW_MagicSpan) return true;
   }
   return false;
}
...
{
   // PATCH: entry fallback to current price if missing/zero
   double __entry_fallback = isBuy ? SymbolInfoDouble(_Symbol, SYMBOL_ASK)
...
{
   int curBar = (int)iBarShift(_Symbol,(ENUM_TIMEFRAMES)Period(),TimeCurrent(),true);
   if(g_LastOpenBar!=-1 && (g_LastOpenBar - curBar) < g_MinBarsBetween)
      return false;
   return true;
}
...
int CountOpenAndPendingAcrossMagics(string sym, long baseMagic, int magicSpan){
int n = 0;
   // Open positions
   int pTot = (int)PositionsTotal();
   for(int i=0;i<pTot;i++){
      ulong ticket = PositionGetTicket(i); if(ticket==0) continue;
      if(!PositionSelectByTicket(ticket)) continue;
      if((string)PositionGetString(POSITION_SYMBOL)!=sym) continue;
      long mg = (long)PositionGetInteger(POSITION_MAGIC);
      if(mg>=baseMagic && mg<=baseMagic+magicSpan) n++;
}

   // Pending / market orders not yet positioned
   int oTot = (int)OrdersTotal();
   for(int j2=0; j2<oTot; j2++){
      ulong oticket = OrderGetTicket(j2);
      if(oticket==0) continue;
      if(!OrderSelect(oticket)) continue;
      if((string)OrderGetString(ORDER_SYMBOL) != sym) continue;
      long mg2 = (long)OrderGetInteger(ORDER_MAGIC);
      if(mg2>=baseMagic && mg2<=baseMagic+magicSpan) n++;
   }
   return n;
}
...
void ComputeStopsPrices(bool isBuy,double &sl,double &tp){
   ...
   if(slp<=0 || tpn<=0){
      slp = MathMax(1, PipsToPoints(g_FixedSL_Pips>0?g_FixedSL_Pips:5));
      tpn = MathMax(1, PipsToPoints(g_FixedTP_Pips>0?g_FixedTP_Pips:10));
   }
...
// ===== Safety Gates (safe defaults, keep existing inputs unchanged) =====
input bool   SkipFirstWeek_input     = true;   // skip trading during days 1..7 each month (NFP week proxy)
input double DailyLossLimitPct_input = 2.5;    // stop trading for the day if drawdown from day start >= 2.5%
input int    LossStreakPause_input   = 2;      // pause trading after N consecutive losses
input int    CooldownMinutes_input   = 10;     // min minutes between opens

double  g_dayStartEquity = 0.0;
int     g_dayStartY=0, g_dayStartM=0, g_dayStartD=0;
int     g_lossStreak = 0;
datetime g_lastOpenTime = 0;

bool IsSameYMD(int y,int m,int d){ MqlDateTime t; TimeToStruct(TimeCurrent(), t); return (t.year==y && t.mon==m && t.day==d); }
void RefreshDayBaselineIfNeeded(){
   if(g_dayStartEquity<=0 || !IsSameYMD(g_dayStartY,g_dayStartM,g_dayStartD)){
      g_dayStartEquity = AccountInfoDouble(ACCOUNT_EQUITY);
      MqlDateTime t; TimeToStruct(TimeCurrent(), t);
      g_dayStartY=t.year; g_dayStartM=t.mon; g_dayStartD=t.day;
      g_lossStreak = 0;
   }
}
bool GateNotFirstWeek(){
   if(!SkipFirstWeek_input) return true;
   MqlDateTime t; TimeToStruct(TimeCurrent(), t);
   return (t.day > 7);
}
bool GateDailyRiskAndCooldown(){
   RefreshDayBaselineIfNeeded();
   double eq = AccountInfoDouble(ACCOUNT_EQUITY);
   if(g_dayStartEquity>0){
      double dd = (g_dayStartEquity - eq)/g_dayStartEquity*100.0;
      if(dd >= DailyLossLimitPct_input) return false;
   }
   if(g_lossStreak >= LossStreakPause_input) return false;
   if(g_lastOpenTime>0 && (TimeCurrent() - g_lastOpenTime) < CooldownMinutes_input*60) return false;
   return true;
}
void TryOpenSignal(){

   
   // --- Global anti-stampede gates (Gatekeeper patch) ---
   if(OpenOnNewBarOnly && !IsNewBarEdge()) { if(g_DebugMode) Print("DEBUG: new-bar only"); return; }
   if(!GateOnePerBarGlobal()) { if(g_DebugMode) Print("DEBUG: one-per-bar global"); return; }
   if(!GateDailyCap()) { if(g_DebugMode) Print("DEBUG: daily max trades"); return; }
   // --- End gates ---
...
if(!SpreadOK()) { if(g_DebugMode) Print("DEBUG: spread high"); return; }
   ...
   if(!GateNotFirstWeek()) { if(g_DebugMode) Print("DEBUG: NFP week skip"); return; }
   if(!GateDailyRiskAndCooldown()) { if(g_DebugMode) Print("DEBUG: daily/cooldown gate"); return; }
   ...
   if(wantBuy && g_LongsAllowed){
      ...
      double sl=0,tp=0; ComputeStopsPrices(true,sl,tp);
         if(sl<=0 || tp<=0){ // safety fallback to fixed
            double entry=SymbolInfoDouble(_Symbol,SYMBOL_ASK);
            double slp=PipsToPoints(g_FixedSL_Pips), tpn=PipsToPoints(g_FixedTP_Pips);
            if(slp<=0) slp=PipsToPoints( (g_FixedSL_Pips>0)? g_FixedSL_Pips:5 );
            if(tpn<=0) tpn=PipsToPoints( (g_FixedTP_Pips>0)? g_FixedTP_Pips:10 );
            sl=entry - slp*_Point; tp=entry + tpn*_Point;
         }
      ...
      if(sl>0 && tp>0){
         ...
         // --- ORCH pretrade gate (BUY) ---
         if(ORCH_ORCH_Enabled){
            double atr_pips = (ATRval((g_StrategyMode==0?PERIOD_M15:PERIOD_H1), g_ATR_Period, 0)/_Point)/DigitsPips();
            double spread_pips = (double)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) / (double)DigitsPips();
            string req_id="", regime="", notes="";
            double rr=0.0, slmin=0.0, slmax=0.0;
            bool ok_pre = ORCH_PretradeCheck("long", atr_pips, spread_pips, ORCH_SessionName(), false, "<bars>", req_id, rr, slmin, slmax, regime, notes);
            if(!ok_pre){
               if(g_DebugMode) Print("ORCH: pretrade deny BUY");
               return;
            }
            // adjust to recommended SL/TP based on rr & slmin
            double ask = SymbolInfoDouble(_Symbol, SYMBOL_ASK);
            double sl_pts = slmin * DigitsPips();
            sl = ask - sl_pts * _Point;
            tp = ask + (slmin*rr*DigitsPips()) * _Point;
            slpts = sl_pts;
            ORCH_last_req_id = req_id;
            ORCH_last_regime = regime;
            ORCH_last_rr = rr;
            ORCH_last_sl_pips = slmin;
            ORCH_last_tp_pips = slmin*rr;
         }
         ...
         AdjustStopsForMinDistance(true, SymbolInfoDouble(_Symbol, SYMBOL_ASK), sl, tp);
          bool ok=false;
          if(!GateMinBarsBetween()) { ok=false; } else {
             if(!BeginOpenAttempt()){ return; }
RoundToTick(sl); RoundToTick(tp);
...
if(!ok){ if(g_DebugMode) PrintFormat("BUY FAIL err=%u rc=%d",GetLastError(),trade.ResultRetcode()); ResetLastError(); }
            ...
            if(g_DebugMode) PrintFormat("BUY OK lots=%.2f pb=%.3f base=%d",lots,pb,baseSig);
            // Ensure SL/TP are actually set (some brokers reject at open)
            if(PositionSelect(_Symbol)){
               double curSL=PositionGetDouble(POSITION_SL);
               double curTP=PositionGetDouble(POSITION_TP);
               double priceOpen=PositionGetDouble(POSITION_PRICE_OPEN);
               if(curSL<=0 || curTP<=0){
                  double _sl=sl, _tp=tp;
                  AdjustStopsForMinDistance(true, priceOpen, _sl, _tp);
                  CTrade t2; t2.SetExpertMagicNumber(g_MagicNumber);
                  t2.PositionModify(_Symbol, _sl, _tp);
               }
            }
          }
   ...
   if(wantSell && g_ShortsAllowed){
      ...
      if(sl>0 && tp>0){
         ...
         if(slpts<=0) slpts=PipsToPoints(g_FixedSL_Pips);
         // --- ORCH pretrade gate (SELL) ---
         if(ORCH_ORCH_Enabled){
            double atr_pips = (ATRval((g_StrategyMode==0?PERIOD_M15:PERIOD_H1), g_ATR_Period, 0)/_Point)/DigitsPips();
            double spread_pips = (double)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD) / (double)DigitsPips();
            string req_id="", regime="", notes="";
            double rr=0.0, slmin=0.0, slmax=0.0;
            bool ok_pre = ORCH_PretradeCheck("short", atr_pips, spread_pips, ORCH_SessionName(), false, "<bars>", req_id, rr, slmin, slmax, regime, notes);
            if(!ok_pre){
               if(g_DebugMode) Print("ORCH: pretrade deny SELL");
               return;
            }
            // adjust to recommended SL/TP based on rr & slmin
            ...
            tp = bid - (slmin*rr*DigitsPips()) * _Point;
            slpts = sl_pts;
            ORCH_last_req_id = req_id;
            ORCH_last_regime = regime;
            ORCH_last_rr = rr;
         ...
         AdjustStopsForMinDistance(false, SymbolInfoDouble(_Symbol, SYMBOL_BID), sl, tp);
          bool ok=false;
          if(!GateMinBarsBetween()) { ok=false; } else {
             if(!BeginOpenAttempt()){ return; }
RoundToTick(sl); RoundToTick(tp);
...
if(!ok){ if(g_DebugMode) PrintFormat("SELL FAIL err=%u rc=%d",GetLastError(),trade.ResultRetcode()); ResetLastError(); }
            ...
            if(g_DebugMode) PrintFormat("SELL OK lots=%.2f ps=%.3f base=%d",lots,ps,baseSig);
            if(PositionSelect(_Symbol)){
               double curSL=PositionGetDouble(POSITION_SL);
               double curTP=PositionGetDouble(POSITION_TP);
               double priceOpen=PositionGetDouble(POSITION_PRICE_OPEN);
               if(curSL<=0 || curTP<=0){
                  double _sl=sl, _tp=tp;
                  AdjustStopsForMinDistance(false, priceOpen, _sl, _tp);
                  CTrade t2; t2.SetExpertMagicNumber(g_MagicNumber);
                  t2.PositionModify(_Symbol, _sl, _tp);
               }
            }
...
int OnInit(){
   ...
   // PATCH: init ORCH runtime flag from input and disable in tester
   ORCH_ORCH_Enabled_runtime = true;
   #ifdef __MQL5_TESTER
      ORCH_ORCH_Enabled_runtime = false;
   #else
      ORCH_ORCH_Enabled_runtime = ORCH_ORCH_Enabled;
   #endif

   
   if(IsTestingOrOpt()) { ORCH_ORCH_Enabled_runtime = false; }
...
UpdateNextLotBoost();
   ...
   if(ORCH_ORCH_Enabled && ORCH_ORCH_DriverMode){
      ORCH_PullDecision_AndMaybeTrade();
   } else {
      TryOpenSignal();
   }
...
void OnTradeTransaction(const MqlTradeTransaction& trans,const MqlTradeRequest& req,const MqlTradeResult& res){
   ...
   if(entry==DEAL_ENTRY_IN){
      ...
      if(ORCH_ORCH_Enabled){
         ...
         if(PositionSelectByTicket(pos_id)){
            ...
            double sl_pips = MathAbs((isBuy? (price - slp): (slp - price)))/_Point / (double)DigitsPips();
            double tp_pips = MathAbs((isBuy? (tpp - price): (price - tpp)))/_Point / (double)DigitsPips();
            double spread_pips = (double)SymbolInfoInteger(_Symbol, SYMBOL_SPREAD)/(double)DigitsPips();
            double atr_pips = (ATRval((g_StrategyMode==0?PERIOD_M15:PERIOD_H1), g_ATR_Period, 0)/_Point)/(double)DigitsPips();
            string tf = ORCH_TimeframeToStr(Period());
            ORCH_TradeLogOpen(ORCH_last_req_id, _Symbol, tf, side, lots, sl_pips, tp_pips,
                               ORCH_last_rr, ORCH_last_regime, spread_pips, atr_pips, ORCH_SessionName(), "",
                               (ulong)pos_id, price);
         }
      }
   ...
   if(entry==DEAL_ENTRY_OUT){
      // ORCH log CLOSE
      if(ORCH_ORCH_Enabled){
         bool isBuy = (dtype==DEAL_TYPE_BUY); // dtype from above refers to this deal; for OUT it is side of exit
         string side = isBuy ? "long" : "short";
         double price_close = HistoryDealGetDouble(deal, DEAL_PRICE);
         double profit = HistoryDealGetDouble(deal, DEAL_PROFIT);
         if(profit>0) g_lossStreak=0; else if(profit<0) g_lossStreak++;
         double pnl_pips = 0.0;
         if(HistorySelectByPosition(pos_id)){
            // rough duration not trivial; leave 0 or compute from order times if needed
         }
         ORCH_TradeLogClose(ORCH_last_req_id, _Symbol, side, (ulong)pos_id, price_close, profit, pnl_pips, 0.0);
      }
...
input bool ORCH_GatekeeperHardFail = true; // deny trading when pretrade HTTP fails
...
input int    MSW_MagicSpan              = 9;      // count positions across baseMagic..baseMagic+span
...
if(!ORCH_ORCH_Enabled) return;
   ...
   string ts_iso   = TimeToString(ts, TIME_DATE|TIME_SECONDS) + "Z";
...
string resp;
bool ok=false;
for(int __i=0; __i<2 && !ok; __i++){
   ok = ORCH_HttpPostJson(ORCH_ORCH_ApiBase + "/heartbeat", payload, resp);
   if(!ok) Sleep(200);
}
...
bool ORCH_PullDecision_AndMaybeTrade()
{
   ...
   int totalOpenNow = CountOpenAndPendingAcrossMagics(sym, g_MagicNumber, (g_MSW_UseSwitch? MSW_MagicSpan:0));
   if(totalOpenNow>=g_MaxOpenPositions){ ORCH_lastDecisionTick=GetTickCount(); return false; }
...
if(!GateMinBarsBetween()){ return false; }
   if(OpenOnNewBarOnly && !IsNewBarEdge()) return false;
   if(!GateOnePerBarGlobal()) return false;
   if(!GateDailyCap()) return false;
ORCH_trade.SetAsyncMode(false);
if(!BeginOpenAttempt()){ return false; }
...
// --- ORCHESTRATOR: Pretrade+Log additions ---
string ORCH_TimeframeToStr(int tf){
   switch(tf){
      ...
      case PERIOD_D1: return "D1"; default: return IntegerToString(tf);
   }
}
string ORCH_SessionName()
{
   ...
   int hour_ = (int)dt.hour;
   if(hour_ >= 7 && hour_ < 10) return "Frankfurt";
   if(hour_ >= 9 && hour_ < 17) return "London";
   if(hour_ >= 13 && hour_ < 22) return "NY";
   return "Asia";
}
bool ORCH_PretradeCheck(const string signal, double atr_pips, double spread_pips, const string session, bool news30m,
                        const string bars, string &request_id, double &rr, double &slmin, double &slmax, string &regime, string &notes)
{
   if(!ORCH_ORCH_Enabled) return true; // if disabled, approve
   string tf = ORCH_TimeframeToStr(Period());
   string body = StringFormat("{\"pair\":\"%s\",\"tf\":\"%s\",\"signal\":\"%s\",\"spread_pips\":%.2f,\"atr_pips\":%.2f,\"session\":\"%s\",\"news_30m\":%s,\"recent_bars_compact\":\"%s\"}",
      _Symbol, tf, signal, spread_pips, atr_pips, session, (news30m?"true":"false"), bars);
   string resp;
   if(!ORCH_HttpPostJson(ORCH_ORCH_ApiBase + "/pretrade-check", body, resp)) { Print("ORCH_PretradeCheck HTTP fail ", resp); return false; }
   string decision = ORCH_JsonGet(resp, "decision");
   regime = ORCH_JsonGet(resp, "regime");
   string rr_s = ORCH_JsonGet(resp, "rr_target");
   rr = (rr_s==""? 0.0 : StringToDouble(rr_s));
   string slr = ORCH_JsonGet(resp, "sl_pips_range");
   // parse [a,b]
   int lb = StringFind(slr,"["); int rb = StringFind(slr,"]");
   double a=0,b=0; if(lb>=0 && rb>lb){ string inside=StringSubstr(slr, lb+1, rb-lb-1); int c=StringFind(inside,",");
      if(c>=0){ a=StringToDouble(Trim(StringSubstr(inside,0,c))); b=StringToDouble(Trim(StringSubstr(inside,c+1))); }
      else { a=StringToDouble(Trim(inside)); b=a; }
   }
   slmin = a; slmax = b;
   notes = ORCH_JsonGet(resp, "notes");
   request_id = ORCH_JsonGet(resp, "request_id");
   bool ok = (decision=="approve");
   if(ok){
      MarkTradeOpened();
      ORCH_last_req_id = request_id;
      ORCH_last_regime = regime;
      ORCH_last_rr = rr;
      ORCH_last_sl_pips = slmin;
      ORCH_last_tp_pips = slmin* (rr<=0?2.0:rr);
   }
   return ok;
}
bool ORCH_TradeLogOpen(const string request_id, const string symbol, const string tf, const string side,
                       ...
                       ulong ticket_id, double price_open)
{
   string body = StringFormat("{\"phase\":\"open\",\"request_id\":\"%s\",\"symbol\":\"%s\",\"tf\":\"%s\",\"side\":\"%s\",\"lots\":%.2f,\"sl_pips\":%.1f,\"tp_pips\":%.1f,\"rr_target\":%.2f,\"regime\":\"%s\",\"decision\":\"approve\",\"spread_pips\":%.2f,\"atr_pips\":%.2f,\"session\":\"%s\",\"notes\":\"%s\",\"ticket_id\":%I64u,\"price_open\":%.5f}",
      request_id, symbol, tf, side, lots, sl_pips, tp_pips, rr_target, regime, spread_pips, atr_pips, session, notes, ticket_id, price_open);
   string resp;
   return ORCH_HttpPostJson(ORCH_ORCH_ApiBase + "/trade-log", body, resp);
}
bool ORCH_TradeLogClose(const string request_id, const string symbol, const string side, ulong ticket_id,
                        double price_close, double pnl_currency, double pnl_pips, double duration_s)
{
   string body = StringFormat("{\"phase\":\"close\",\"request_id\":\"%s\",\"symbol\":\"%s\",\"side\":\"%s\",\"ticket_id\":%I64u,\"price_close\":%.5f,\"pnl_currency\":%.2f,\"pnl_pips\":%.1f,\"duration_s\":%.0f}",
      request_id, symbol, side, ticket_id, price_close, pnl_currency, pnl_pips, duration_s);
   string resp;
   return ORCH_HttpPostJson(ORCH_ORCH_ApiBase + "/trade-log", body, resp);
}
// --- END additions ---
